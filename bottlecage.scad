...
// https://github.com/gutschke/bottlecage/

$fa = 5;
$fs = 5.4;

hasslot = true;         // Should there be multiple adjustable mount points?

// The following adjustable parameters are in millimeters
diameterin = 73.0250;     // Diameter of bottle (common: 2.875" and 2.6")
bracketheightin = 127;    // Height of bracket
bracketwidthin = 25.4;     // Width of bracket attaching to bike frame
brackettopin = 2.54;     // Radius of reinforcements at top of bracket   
brackettaperin = 1.9; // Bracket tapers in by a few millimeters
screwdistancein = 63.5;  // Distance between primary mounting holes
heightin = 152.4;           // Overall height of entire bottle cage
framediameterin = 44.45; // Diameter of bicycle frame
...
module bottlecage_() {
  difference() {
    ...
    union() {
      ...
      linear_extrude(height = wallthick)
        difference() {
          ...
          polygon([[bracketthickness/4,  bracketwidth/2],
                   ...
                   [outerdiameter - 0.4 - bracethickness - wallthin,  wallthick/2],
                   [outerdiameter - 0.4 - wallthin,  wallthick/2],
                   [outerdiameter - 0.4 - wallthin, -wallthick/2],
                   [outerdiameter - 0.4 - bracethickness - wallthin, -wallthick/2],
                   [bracketthickness, -bracketwidth/2],
      ...
      intersection() {
        union() {
          ...
          translate([outerdiameter - 0.4 - wallthin - bracethickness + wallthick, 0, wallthick])
            rotate([90, 0, 0])
            cylinder(r = bracethickness, h = wallthick, center = true);
          
          // Vertical part of the brace  
          translate([(outerdiameter + diameter)/2 - 0.4 + 1.5*wallthick - wallthin/2 - bracethickness, -wallthick/2, wallthick])
            cube([bracethickness, wallthick, toploopoffset]);
          }
    ...
    hull() {
      for (z = [wallthick + (bracketheight - screwdistance)/2,
                each [if (hasslot) bracketwidth/2 + wallthick]])
        translate([wallthick - screwdiameter/2, 0, z])
          rotate([0, 90, 0])
            cylinder(r = screwdiameter + 0.25, h = bracketthickness);
    }
    ...
    union() {
      difference() {
        ...
        translate([outerdiameter - 0.4 - wallthin - wallthick,
                   -5*wallthick, wallthick])
          cube([wallthick, 10*wallthick, wallthick]);
      }
      ...
      hull() {
        translate([outerdiameter - 0.4 - wallthick - wallthin, -wallthick/2,
                   2*wallthick])
          rotate([-90, 0, 0])
